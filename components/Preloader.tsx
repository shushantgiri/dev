...
const Preloader = () => {
    ...
    // Uses your first name (or first word of GENERAL_INFO.name) for the
    // big animated preloader text. Falls back to "HELLO" if name is empty.
    const preloaderText = (
        GENERAL_INFO.firstName || GENERAL_INFO.name.split(' ')[0] || 'HELLO'
    ).toUpperCase();
    ...
    return (
        <div className="fixed inset-0 z-[6] flex" ref={preloaderRef}>
            ...
            <p className="name-text flex text-[20vw] lg:text-[200px] font-anton text-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 leading-none overflow-hidden">
                {preloaderText.split('').map((letter, idx) => (
                    <span
                        key={`${letter}-${idx}`}
