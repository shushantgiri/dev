...
const Project = ({ index, project, selectedProject, onMouseEnter }: Props) => {
    ...
    return (
        <TransitionLink
            ...
            className="project-item group leading-none py-6 px-4 -mx-4 rounded-2xl md:border-b md:rounded-none first:!pt-0 last:pb-6 last:border-none md:group-hover/projects:opacity-30 md:hover:!opacity-100 md:hover:bg-background-light/60 transition-all duration-500"
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
        >
            {selectedProject === null && (
                <div className="w-full mb-6 overflow-hidden rounded-xl aspect-[3/2]">
                    <Image
            ...
            <div className="flex gap-2 md:gap-5">
                ...
                <div className="flex-1">
                    <div className="flex items-center flex-wrap gap-x-4 gap-y-1">
                        ...
                        {project.year && (
                            <span className="text-sm text-muted-foreground border border-border/80 rounded-full px-2.5 py-1 leading-none">
                                {project.year}
                            </span>
                        )}
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2">
                        {project.techStack.slice(0, 3).map((tech) => (
                            <span
                                key={tech}
                                className="text-xs text-muted-foreground bg-background-light/80 rounded-full px-3 py-1.5 leading-none"
                            >
                                {tech}
                            </span>
                        ))}
